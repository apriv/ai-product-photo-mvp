...
> 状态：v1 已完成。本文档保留设计思路和当前实现约定，后续 v2 继续做产品化打磨。

## 1. 目标

把早期的"整站共享密码"换成正式的用户系统，支持：
...
- **登录鉴权**：`src/proxy.ts` 做路由保护，route handler 内用 `requireUser()` / `requireAdmin()` 做真实鉴权。
- **用户工具**：`src/lib/auth.ts` 管理 session、密码哈希、当前用户读取。
- **积分工具**：`src/lib/credits.ts` 管理扣费、退款、余额和到期校验。
- **激活码工具**：`src/lib/activation.ts` 和 `src/lib/activation-codes.ts` 管理激活码消费和生成。
- **图片扣费**：`/api/image/generate` 根据模板 cost 扣费，失败时退款。
- **模板价格**：`src/features/image/templates.ts` 是图片模板和积分价格的主要配置入口。
- **账户页**：`/account` 展示余额、套餐状态和流水。
